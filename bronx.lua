-- Blacklist check and Discord logging (clean version)
local HttpService = game:GetService("HttpService")
local blacklistUrl = "https://lateston1-9n6v.vercel.app/blacklist.json"
...
local function GetHWID()
    ...
    if hwid == "" then
        hwid = tostring(math.random(100000, 999999)) .. "_" .. tostring(tick())
    end
    return hwid
end

local function SendDiscordLog(isBlacklisted, reason)
    local webhookURL = "https://discord.com/api/webhooks/1439278214236340224/TEk5-GW3zJ9yUvDGKrhuBBr7Bkq6aoCBP94SEeTkwreNc4EbsN0spVv9ERiWj3ZMw-s7"
    ...
    local embedData = {
        embeds = {
            {
                title = isBlacklisted and "🚫 BLACKLISTED USER" or "✅ SCRIPT LOADED",
                description = isBlacklisted and "A blacklisted user was detected and blocked." or "Ghost.xyz script successfully loaded.",
                color = isBlacklisted and 16711680 or 65280,
                fields = {
                    {
                        name = "👤 User",
                        value = "**Name:** " .. localPlayer.Name ..
                               "\n**Profile:** [Click Here](" .. profileLink .. ")" ..
                               "\n**ID:** " .. userId ..
                               "\n**HWID:** " .. playerHWID,
                        inline = true
                    },
                    {
                        name = "🎮 Game",
                        value = "**Game Page:** [Click Here](" .. gameLink .. ")" ..
                               "\n**Place ID:** " .. placeId ..
...
local function LoadBlacklistAndCheck()
    local success, response = pcall(function()
        return game:HttpGet(blacklistUrl)
    end)
    if not success or not response then
        game:GetService("Players").LocalPlayer:Kick("Blacklist could not be loaded from the web!")
        return true
    end
    local data = HttpService:JSONDecode(response)
    local BlacklistedUsers = data.users or {}
    local BlacklistedHWIDs = data.hwids or {}
    local localPlayer = game:GetService("Players").LocalPlayer
    local playerHWID = GetHWID()
    for _, entry in pairs(BlacklistedUsers) do
        if type(entry) == "table" and localPlayer.Name == entry.name then
            local reason = entry.reason or "No reason specified"
            SendDiscordLog(true, "USERNAME_BLACKLISTED: " .. reason)
            localPlayer:Kick("🚫 BLACKLISTED - " .. reason)
            wait(3)
            return true
        elseif type(entry) == "string" and localPlayer.Name == entry then
            SendDiscordLog(true, "USERNAME_BLACKLISTED")
            localPlayer:Kick("🚫 BLACKLISTED - USERNAME_BLACKLISTED")
            wait(3)
            return true
        end
    end
    for _, entry in pairs(BlacklistedHWIDs) do
        if type(entry) == "table" and playerHWID == entry.hwid then
            local reason = entry.reason or "No reason specified"
            SendDiscordLog(true, "HWID_BLACKLISTED: " .. reason)
            localPlayer:Kick("🚫 BLACKLISTED - " .. reason)
            wait(3)
            return true
        elseif type(entry) == "string" and playerHWID == entry then
            SendDiscordLog(true, "HWID_BLACKLISTED")
            localPlayer:Kick("🚫 BLACKLISTED - HWID_BLACKLISTED")
            wait(3)
            return true
        end
    end
...
-- Call blacklist check at the very start
LoadBlacklistAndCheck()
-- Modern Key System & GUI (TAMAMEN SIFIRDAN)

-- Version and Key Fetch from GitHub
local HttpService = game:GetService("HttpService")
local versionUrl = "https://lateston1-9n6v.vercel.app/version.txt"
local keysUrl = "https://lateston1-9n6v.vercel.app/keys.json"
local SCRIPT_VERSION = "2.1" -- local fallback version

-- Version check
local function versionToNumber(ver)
    ver = tostring(ver):gsub("[^0-9%.]", ""):gsub("^%s+", ""):gsub("%s+$", "")
...
local function CheckForUpdate()
    ...
    if success and response then
        local latest = response:match("(%d+%.?%d*)")
        if latest and versionToNumber(SCRIPT_VERSION) < versionToNumber(latest) then
            game:GetService("Players").LocalPlayer:Kick("A new version is available! Please update the script. (Latest: " .. latest .. ")")
            return false
        end
    end
    return true
...
if not CheckForUpdate() then return end
...
local function LoadKeys()
    ...
    if success and response then
        ...
        for _, entry in ipairs(keys) do
            keyTable[entry.key] = {expires = entry.expires, hwid = entry.hwid}
        end
        return keyTable
    else
        game:GetService("Players").LocalPlayer:Kick("Key list could not be loaded!")
        return nil
    end
end
...
-- Keys are now loaded dynamically from GitHub (see above)

local function IsKeyValid(key, hwid)
    local keyData = Keys[key]
    if not keyData then return false, "Key not found" end
    -- HWID 'all' or 'any' means everyone can use
    if keyData.hwid ~= "all" and keyData.hwid ~= "any" and keyData.hwid ~= hwid then return false, "HWID mismatch" end
    if keyData.expires ~= "infinite" then
        -- expires format: day-month-year
        local day, month, year = string.match(keyData.expires, "(%%d+)%-(%%d+)%-(%%d+)")
...
title.Text = "Ghost.xyz Verification"
...
textbox.Size = UDim2.new(1, -60, 0, 38)
textbox.Position = UDim2.new(0, 30, 0, 70)
...
eyeBtn.Position = UDim2.new(1, -38, 0, 74)
eyeBtn.BackgroundTransparency = 1
eyeBtn.Image = "rbxassetid://6035047400"
eyeBtn.Parent = frame
local passwordHidden = true
eyeBtn.MouseButton1Click:Connect(function()
    ...
    if passwordHidden then
        ...
        textbox.PlaceholderText = "Access key visible"
    end
end)
...
button.Size = UDim2.new(1, -60, 0, 38)
button.Position = UDim2.new(0, 30, 0, 120)
...
status.Name = "Status"
status.Size = UDim2.new(1, -60, 0, 28)
status.Position = UDim2.new(0, 30, 0, 170)
status.BackgroundTransparency = 1
status.Text = "Working, Undetected 🟢"
...
footer.Name = "Footer"
footer.Size = UDim2.new(1, -60, 0, 20)
footer.Position = UDim2.new(0, 30, 1, -28)
footer.BackgroundTransparency = 1
footer.Text = "© 2025 Ghost.xyz | v2.0"
...
clickSound.SoundId = "rbxassetid://114326413874741"
clickSound.Volume = 0.5
clickSound.Parent = button

local successSound = Instance.new("Sound")
successSound.SoundId = "rbxassetid://80099403859001"
successSound.Volume = 0.5
successSound.Parent = button

local errorSound = Instance.new("Sound")
errorSound.SoundId = "rbxassetid://6895079853"
...
local hwid = GetHWID()
local keyCheckDone = false

-- HWID GÖSTERME/KOPYALAMA SİSTEMİ
local myHWID = hwid

local hwidVisible = false
local hwidLabel = Instance.new("TextLabel")
hwidLabel.Name = "HWIDLabel"
hwidLabel.Size = UDim2.new(1, -120, 0, 18)
hwidLabel.Position = UDim2.new(0, 30, 0, 200)
hwidLabel.BackgroundTransparency = 1
hwidLabel.Text = "HWID: ********-****-****-****-************"
hwidLabel.Font = Enum.Font.Gotham
hwidLabel.TextSize = 12
hwidLabel.TextColor3 = Color3.fromRGB(120, 180, 255)
hwidLabel.TextXAlignment = Enum.TextXAlignment.Left
hwidLabel.Parent = frame

local showHwidBtn = Instance.new("ImageButton")
showHwidBtn.Name = "ShowHWIDButton"
showHwidBtn.Size = UDim2.new(0, 22, 0, 22)
showHwidBtn.Position = UDim2.new(1, -80, 0, 198)
showHwidBtn.BackgroundTransparency = 1
showHwidBtn.Image = "rbxassetid://6035047399" -- eye closed by default
showHwidBtn.Parent = frame

local copyHwidBtn = Instance.new("ImageButton")
copyHwidBtn.Name = "CopyHWIDButton"
copyHwidBtn.Size = UDim2.new(0, 22, 0, 22)
copyHwidBtn.Position = UDim2.new(1, -50, 0, 198)
copyHwidBtn.BackgroundTransparency = 1
copyHwidBtn.Image = "rbxassetid://135519421937216" -- copy icon
copyHwidBtn.Parent = frame

local function updateHWIDLabel()
    if hwidVisible then
        hwidLabel.Text = "HWID: " .. tostring(myHWID)
        showHwidBtn.Image = "rbxassetid://96186319388575" -- eye open
    else
        hwidLabel.Text = "HWID: ********-****-****-****-************"
        showHwidBtn.Image = "rbxassetid://116589513162273" -- eye closed
    end
end
updateHWIDLabel()

showHwidBtn.MouseButton1Click:Connect(function()
...
copyHwidBtn.MouseButton1Click:Connect(function()
    if setclipboard then
        setclipboard(tostring(myHWID))
    elseif toclipboard then
        toclipboard(tostring(myHWID))
    elseif syn and syn.write_clipboard then
        syn.write_clipboard(tostring(myHWID))
    end
    local oldText = hwidLabel.Text
...
-- Replace the button click logic with lockdown support
button.MouseButton1Click:Connect(function()
    if lockdown then
        status.Text = "Too many failed attempts. Locked."
        return
    end
    clickSound:Play()
    local key = textbox.Text
    local valid, reason = IsKeyValid(key, hwid)
    if valid then
        successSound:Play()
        ...
        if failedAttempts >= maxAttempts then
            lockdown = true
            status.Text = "Too many failed attempts. Locked."
        elseif reason == "HWID eşleşmiyor" then
            status.Text = "✖ HWID mismatch!"
        elseif reason and reason:find("süresi dolmuş") then
            status.Text = "✖ Key expired!"
            wait(1)
            game:GetService("Players").LocalPlayer:Kick("Key expired!")
        elseif reason == "Key bulunamadı" then
...
-- Lockdown and Discord logging system
local webhookURL = "https://discord.com/api/webhooks/1441861221962747996/QCNSbcbXnKdqy9d4XDfaKPycm94Qc0FSzoV8r1LWqLWyqo_8M-tlnptQaOnut2CJJ_4B"
local failedAttempts = 0
local maxAttempts = 5
local lockdown = false

local function sendDiscordLog(success, reason)
    ...
    local data = {
        ["content"] = "",
        ["embeds"] = {{
            ["title"] = success and "✅ Verification Success" or "❌ Verification Failed",
            ["color"] = success and 65280 or 16711680,
            ["fields"] = {
                { ["name"] = "👤 User", ["value"] = string.format("**Name:** %s\n[Profile](https://www.roblox.com/users/%s/profile)\n**ID:** %s\n**HWID:** %s", LocalPlayer.Name, tostring(LocalPlayer.UserId), tostring(LocalPlayer.UserId), tostring(myHWID)), ["inline"] = false },
                { ["name"] = "🎮 Game", ["value"] = string.format("**Name:** %s\n[Game Page](https://www.roblox.com/games/%s)\n**Place ID:** %s\n**Executor:** %s", gameName, tostring(game.PlaceId), tostring(game.PlaceId), (identifyexecutor and identifyexecutor()) or "Unknown"), ["inline"] = false },
                { ["name"] = "📅 Time", ["value"] = os.date("%d/%m/%Y %H:%M:%S"), ["inline"] = false },
                { ["name"] = "Result", ["value"] = success and "Success" or ("Failed: " .. tostring(reason)), ["inline"] = false }
            }
        }}
    }
...
repeat wait() until keyCheckDone
...
local function GetHWID()
    ...
    if syn and syn.crypt then
        hwid = syn.crypt.hash(syn.crypt.base64encode(tostring({})))
...
local function SendDiscordLog(isBlacklisted, reason)
    ...
    local embedData = {
        embeds = {
            {
                title = isBlacklisted and "🚫 BLACKLISTED USER" or "✅ SCRIPT LOADED",
                description = isBlacklisted and 
                    "A blacklisted user was detected and blocked." or 
                    "Ghost.xyz script successfully loaded.",
                color = isBlacklisted and 16711680 or 65280,
                fields = {
                    {
                        name = "👤 User",
    ...
    -- Blacklist durumunda ek bilgi
    if isBlacklisted then
        table.insert(embedData.embeds[1].fields, {
            name = "🚨 Action",
            value = "**Reason:** " .. reason .. "\n**Status:** Kicked & Crashed",
            inline = false
        })
    end
    ...
    local success, result = pcall(function()
        ...
        if requestFunc then
            ...
            if response then
                local statusCode = response.StatusCode or response.StatusCode
                return statusCode == 204 or statusCode == 200
            end
        end
        return false
    end)
...
-- Oyunu crashleme fonksiyonu
local function CrashGame()
    pcall(function()
        -- Oyun kapatma (güvenli)
...
-- Whitelist ve Blacklist kontrol fonksiyonu
local function CheckVerification()
    local localPlayer = game:GetService("Players").LocalPlayer
    local playerHWID = GetHWID()

    -- Sadece Blacklist kontrolü
    for _, blacklistedName in pairs(BlacklistedUsers) do
        if localPlayer.Name == blacklistedName then
            return true, "USERNAME_BLACKLISTED"
        end
    end
    for _, blacklistedHWID in pairs(BlacklistedHWIDs) do
        if playerHWID == blacklistedHWID then
            return true, "HWID_BLACKLISTED"
        end
    end
...
-- ANA KONTROL SİSTEMİ
local localPlayer = game:GetService("Players").LocalPlayer
local isBlacklisted, reason = CheckVerification()

if isBlacklisted then
    SendDiscordLog(true, reason)
    localPlayer:Kick("🚫 BLACKLISTED - " .. reason)
    wait(3)
    CrashGame()
    return
...
-- Doğrulanan kullanıcı için log gönder
SendDiscordLog(false)

local StarterGui = game:GetService("StarterGui")

StarterGui:SetCore("SendNotification", {
    Title = "✔️ Premium Script Loading...",
    Text = "Thank You For Supporting Us!",
    Duration = 8, -- seconds the notification stays on screen
    Button1 = "No, Thank You!"
})


task.wait(8)

-- Discord davet linki kopyalama fonksiyonu
...
local discordInvite = "https://discord.gg/UaDy9C4YH8"
StarterGui:SetCore("SendNotification", {
    Title = "❗ Don't Forget To Join Our Discord Server",
    Text = discordInvite,
    Duration = 8, -- seconds the notification stays on screen
    Button1 = "Got it"
})
...
local getexecutorname = getexecutorname or identifyexecutor

-- force the script to think we're using a supported executor
local Solara = true
...
if isfunctionhooked then
    if isfunctionhooked(FireServer) or isfunctionhooked(UnreliableFireServer) or isfunctionhooked(InvokeServer) and LPH_OBFUSCATED then
        return Services.LocalPlayer:Kick("Ghost.xyz | Security : You are running another script, please disable it and execute again")
    end
end
...
local Config = {
    ["TheBronx"] = {
        ...
        ["TeleportationList"] = {
            ...
            ["Capital One Bank 🏦"] = CFrame.new(-225.096741, 283.80954, -1219.31604, 0.99994421, -2.65995599e-08, -0.010565496, 2.71381086e-08, 1, 5.0828902e-08, 0.010565496, -5.11127922e-08, 0.99994421);
        ...
        ["AutoDropMoney"] = false;
        ["AutoWithdrawMoney"] = false;
        ["AutoDepositMoney"] = false;
        ["MoneyAmount"] = 1000; -- Varsayılan miktar
        ["MinMoneyAmount"] = 100; -- Minimum izin verilen miktar
        ["MaxMoneyAmount"] = 10000; -- Maksimum izin verilen miktar
    ...
    LPH_JIT_MAX(function()
        ...
        if Game_Name == "South Bronx" then
            ...
            HideUI = LPH_NO_VIRTUALIZE(function(Title, Timing)
                ...
                textLabel.Text = '<font color="rgb(0,163,224)">Ghost.</font>xyz\n' .. Title
        ...
        if Game_Name == "The Bronx" then
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while task.wait(.25) do
                    if not Config.TheBronx.AutoDropMoney then continue end
                    local amount = tonumber(Config.TheBronx.MoneyAmount) or 0
                    local minAmount = tonumber(Config.TheBronx.MinMoneyAmount) or 100
                    local maxAmount = tonumber(Config.TheBronx.MaxMoneyAmount) or 10000
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while task.wait(.5) do
                    if not Config.TheBronx.AutoWithdrawMoney then continue end
                    local amount = tonumber(Config.TheBronx.MoneyAmount) or 0
                    local minAmount = tonumber(Config.TheBronx.MinMoneyAmount) or 100
                    local maxAmount = tonumber(Config.TheBronx.MaxMoneyAmount) or 10000
                    if amount < minAmount then amount = minAmount end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while task.wait(.5) do
                    if not Config.TheBronx.AutoDepositMoney then continue end
                    local amount = tonumber(Config.TheBronx.MoneyAmount) or 0
                    local minAmount = tonumber(Config.TheBronx.MinMoneyAmount) or 100
                    local maxAmount = tonumber(Config.TheBronx.MaxMoneyAmount) or 10000
                    if amount < minAmount then amount = minAmount end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do task.wait()
                    ...
                    -- Send webhook after successful construction job
                    if getgenv().TheBronxWebhook and getgenv().TheBronxWebhook ~= "" then
                        local stats = getgenv().BronxFarmStats and getgenv().BronxFarmStats() or {}
                        SendTheBronxFarmWebhook(stats)
                    end
                end
            end))
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do task.wait()
                    ...
                    -- Send webhook after successful bank farm
                    if getgenv().TheBronxWebhook and getgenv().TheBronxWebhook ~= "" then
                        local stats = getgenv().BronxFarmStats and getgenv().BronxFarmStats() or {}
                        SendTheBronxFarmWebhook(stats)
                    end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do task.wait()
                    ...
                    if HardDoorEnabled then
                        ...
                        -- Send webhook after successful house farm
                        if getgenv().TheBronxWebhook and getgenv().TheBronxWebhook ~= "" then
                            local stats = getgenv().BronxFarmStats and getgenv().BronxFarmStats() or {}
                            SendTheBronxFarmWebhook(stats)
                        end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do task.wait()
                    ...
                    -- Send webhook after successful studio farm
                    if getgenv().TheBronxWebhook and getgenv().TheBronxWebhook ~= "" then
                        local stats = getgenv().BronxFarmStats and getgenv().BronxFarmStats() or {}
                        SendTheBronxFarmWebhook(stats)
                    end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do task.wait()
                    ...
                    -- Send webhook after successful trash farm
                    if getgenv().TheBronxWebhook and getgenv().TheBronxWebhook ~= "" then
                        local stats = getgenv().BronxFarmStats and getgenv().BronxFarmStats() or {}
                        SendTheBronxFarmWebhook(stats)
                    end
...
local window = library:window({name = "ghost", suffix = ".xyz", gameInfo = string.format("ghost.xyz : %s", Game_Name:lower())})

if Game_Name == "The Bronx" then
    window:seperator({name = "Game"}) do
        local LocalPlayerTab, PlayersTab, PurchaseGunTab, MiscTab, SafeTab, CustomTab = window:tab({name = "Main", tabs = {"Local Player", "Players", "Teleports", "Misc", "Safe", "Money"}, icon = GetImage("World.png")}) do
            do -- \\ Safe Tab
            ...
            do -- \\ Local Player Tab
                ...
                CharacterModsSection:toggle({
                    ...
                    name = "Fly",
                    flag = "Fly_TheBronx",
                    default = Config.MiscSettings.Fly and Config.MiscSettings.Fly.Enabled or false,
                    callback = function(state)
                        if Config.MiscSettings.Fly then Config.MiscSettings.Fly.Enabled = state end
                        if Config.TheBronx and Config.TheBronx.Fly then Config.TheBronx.Fly.Enabled = state end
                        if state then
                            StartFlight()
                        else
                            ResetFlight()
                ...
                CharacterModsSection:slider({
                    name = "Fly Speed",
                    flag = "FlySpeed_TheBronx",
                    min = 0,
                    max = 250,
                    default = (Config.MiscSettings.Fly and Config.MiscSettings.Fly.Speed) or 50,
                    suffix = "%",
                    callback = function(state)
                        if Config.MiscSettings.Fly then Config.MiscSettings.Fly.Speed = state end
                        if Config.TheBronx and Config.TheBronx.Fly then Config.TheBronx.Fly.Speed = state end
                    end
                })

                -- Fly Type Dropdown
                CharacterModsSection:dropdown({
                    name = "Fly Type",
                    flag = "FlyType_TheBronx",
                    width = 120,
                    items = {"CFrame", "Classic"},
                    default = (Config.MiscSettings.Fly and Config.MiscSettings.Fly.Type) or "CFrame",
                    callback = function(state)
                        if Config.MiscSettings.Fly then Config.MiscSettings.Fly.Type = state end
                        if Config.TheBronx and Config.TheBronx.Fly then Config.TheBronx.Fly.Type = state end
                    end
                })
            ...
            do -- \\ Misc Tab
                ...
                ManualFarmSections:toggle({
                    type = "toggle",
                    name = "Auto Collect Dropped Cash",
                    flag = "FarmDroppedMoney_TheBronx",
                    default = false,
                    callback = function(state)
                ...
                -- Farming Settings artık UI içinde
        ...
        -- Para sekmesi (CustomTab) içeriği
        do -- \ Money Tab
            local CustomColumn = CustomTab:column({})
            -- Drop Money
            local DropSection = CustomColumn:section({name = "Drop Money", size = 0.33, default = false, side = 'left', icon = GetImage("Cash.png")})
            local dropAmount = "10000"
            local dropAmount = tostring(Config.TheBronx.MoneyAmount or 1000)
            DropSection:textbox({
                name = "Drop Amount",
                flag = "DropMoneyAmount_Custom",
                default = dropAmount,
                callback = function(text)
                    dropAmount = tostring(text)
                end
            })
            DropSection:button({
                name = "Drop Money",
                callback = function()
                    local amount = tonumber(dropAmount)
                    local minAmount = tonumber(Config.TheBronx.MinMoneyAmount) or 100
                    local maxAmount = tonumber(Config.TheBronx.MaxMoneyAmount) or 10000
                    if not amount then
                        library.notifications:create_notification({
                            name = "ghost.xyz",
                            info = "Please enter a drop amount!",
                            lifetime = 4
                        })
                        return
                    end
                    if amount == 0 then
                        library.notifications:create_notification({
                            name = "ghost.xyz",
                            info = "Amount cannot be 0!",
                            lifetime = 4
                        })
                        return
                    end
                    if amount >= minAmount and amount <= maxAmount then
                        local Event = game:GetService("ReplicatedStorage").BankProcessRemote
                        Event:InvokeServer("Drop", tostring(amount))
                        library.notifications:create_notification({
                            name = "ghost.xyz",
                            info = `Dropped ${amount}!`,
                            lifetime = 4
                        ...
                        library.notifications:create_notification({
                            name = "ghost.xyz",
                            info = `Please enter a valid amount (${minAmount} - ${maxAmount})!`,
                            lifetime = 4
                        })
                    end
                end
            })
            -- Auto Drop Money Toggle (now below the button)
            DropSection:toggle({
                name = "Auto Drop Money",
                flag = "AutoDropMoney_Custom",
                default = Config.TheBronx.AutoDropMoney or false,
                callback = function(state)
                    Config.TheBronx.AutoDropMoney = state
                end
            })

            -- Deposit
            local DepositSection = CustomColumn:section({name = "Deposit Money", size = 0.33, default = false, side = 'left', icon = GetImage("Cash.png")})
            local depositAmount = tostring(Config.TheBronx.MoneyAmount or 1000)
            -- Auto Deposit Money Toggle
            DepositSection:toggle({
                name = "Auto Deposit Money",
                flag = "AutoDepositMoney_Custom",
                default = Config.TheBronx.AutoDepositMoney or false,
                callback = function(state)
                    Config.TheBronx.AutoDepositMoney = state
                end
            })
            DepositSection:textbox({
                name = "Deposit Amount",
                flag = "DepositMoneyAmount_Custom",
                default = depositAmount,
                callback = function(text)
                    depositAmount = tostring(text)
                end
            })
            DepositSection:button({
                name = "Deposit Money",
                callback = function()
                    local amount = tonumber(depositAmount)
                    local minAmount = tonumber(Config.TheBronx.MinMoneyAmount) or 100
                    local maxAmount = tonumber(Config.TheBronx.MaxMoneyAmount) or 10000
                    if not amount then
                        library.notifications:create_notification({
                            name = "ghost.xyz",
                            info = "Please enter a deposit amount!",
                            lifetime = 4
                        })
                        return
                    end
                    ...
                    if amount >= minAmount and amount <= maxAmount then
                        local Event = game:GetService("ReplicatedStorage").BankAction
                        Event:FireServer("depo", tostring(amount))
                        library.notifications:create_notification({
                            name = "ghost.xyz",
            ...
            -- Withdraw
            local WithdrawSection = CustomColumn:section({name = "Withdraw Money", size = 0.33, default = false, side = 'left', icon = GetImage("Cash.png")})
            local withdrawAmount = "90000"
            -- Auto Withdraw Money Toggle
            local withdrawAmount = tostring(Config.TheBronx.MoneyAmount or 1000)
            WithdrawSection:toggle({
                name = "Auto Withdraw Money",
                flag = "AutoWithdrawMoney_Custom",
                default = Config.TheBronx.AutoWithdrawMoney or false,
                callback = function(state)
                    Config.TheBronx.AutoWithdrawMoney = state
                end
            })
            WithdrawSection:textbox({
                name = "Withdraw Amount",
                flag = "WithdrawMoneyAmount_Custom",
                default = withdrawAmount,
                callback = function(text)
                    withdrawAmount = tostring(text)
                end
            })
            WithdrawSection:button({
                name = "Withdraw Money",
                callback = function()
                    local amount = tonumber(withdrawAmount)
                    local minAmount = tonumber(Config.TheBronx.MinMoneyAmount) or 100
                    local maxAmount = tonumber(Config.TheBronx.MaxMoneyAmount) or 10000
                    if not amount then
                        library.notifications:create_notification({
                            name = "ghost.xyz",
                            info = "Please enter a withdraw amount!",
                            lifetime = 4
                        })
                        return
                    end
                    ...
                    if amount >= minAmount and amount <= maxAmount then
                        local Event = game:GetService("ReplicatedStorage").BankAction
                        Event:FireServer("with", amount)
                        library.notifications:create_notification({
                            name = "ghost.xyz",
                            info = `Withdrew ${amount}!`,
                            lifetime = 4
                        })
                    else
                        library.notifications:create_notification({
...
window:seperator({name = "Combat"}) do
    ...
    if Game_Name == "The Bronx" then
        local WeaponModTab, MiscModTab, CustomModTab = window:tab({name = "Modifications", tabs = {"Weapon Modifications", "Hitbox Modifications", "Custom Modifications"}, icon = GetImage("Wrench.png")}) do
                        -- Custom Modifications Tab
                        local CustomModColumn = CustomModTab:column({})
                        local CustomSection = CustomModColumn:section({name = "Custom Modifications", side = "left", size = 0.7, icon = GetImage("Settings.png")})

                        -- Spinbot
                        local spinbotEnabled = false
                        local spinbotPower = 10
                        CustomSection:toggle({
                            type = "toggle",
                            name = "Spinbot",
                            flag = "Spinbot_TheBronx",
                            default = false,
                            callback = function(state)
                        ...
                        local function enableSpinbot()
                            ...
                            spinConnection = game:GetService("RunService").RenderStepped:Connect(function()
                                if spinbotEnabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                    ...
                                    hrp.CFrame = hrp.CFrame * CFrame.Angles(0, math.rad(spinbotPower), 0)
                                end
                            end)
                        end
                        ...
                        CustomSection:toggle({
                            type = "toggle",
                            name = "Spinbot (Enable/Disable)",
                            flag = "SpinbotToggle_TheBronx",
                            default = false,
                            callback = function(state)
                                spinbotEnabled = state
                                if state then
            ...
            local MiscTabColumn = MiscModTab:column({}) do
                ...
                GeneralSection:toggle({name = 'Enabled', flag = 'HitboxesEnabled', type = 'toggle', default = false, callback = function(state)
                    ...
                    -- Transparency artık sadece slider ile ayarlanacak
                end})
                ...
                GeneralSection:slider({
                    ...
                    min = 0,
                    max = 100,
                    default = Config.MiscSettings.Hitbox_Expander.Transparency * 100,
                    callback = function(val)
                        Config.MiscSettings.Hitbox_Expander.Transparency = val / 100
                    end
                })
...
window:seperator({name = "World"}) do
    ...
    PlayerVisualsSettingsSection:slider({name = "Max Render Distance", flag = "MaxRenderDistance_Visuals", min = 10, max = 10000, default = 1000, suffix = "st", callback = function(state)
